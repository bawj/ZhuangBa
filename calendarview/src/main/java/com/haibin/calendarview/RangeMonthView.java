...
public abstract class RangeMonthView extends BaseMonthView {
    ...
    public RangeMonthView(Context context) {
        ...
        titlePaint.setTextSize(CalendarUtil.dipToPx(context, TEXT_SIZE));
    ...
    private void draw(Canvas canvas, Calendar calendar, int i, int j) {
        ...
        int y = i * mItemHeight + mItemWidth / 2;

        if (!isDraw){
            int textWidth = mItemWidth / 2;
            int textHeight = mItemHeight / 2;
            String text = calendar.getYear() + "年" + calendar.getMonth() + "月";
            canvas.drawText(text, textWidth, textHeight, titlePaint);

            canvas.drawLine(textWidth, (mLineCount + 1) * mItemHeight, mItemWidth * 7, (mLineCount + 1) * mItemHeight, linePaint);
            isDraw = true;
