...
public class CodeEditText extends AppCompatEditText {
    ...
    private void setAttrs(AttributeSet attrs) {
        TypedArray t = mC.obtainStyledAttributes(attrs, R.styleable.CodeEditText);
        if (t != null) {
            textLength = t.getInt(R.styleable.CodeEditText_textLength, 6);
            spzceX = t.getDimensionPixelSize(R.styleable.CodeEditText_space, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 3, getResources().getDisplayMetrics()));
            spzceY = t.getDimensionPixelSize(R.styleable.CodeEditText_space, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 3, getResources().getDisplayMetrics()));
            StrokeWidth = t.getDimensionPixelSize(R.styleable.CodeEditText_strokeWidth, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, getResources().getDisplayMetrics()));
            Round = t.getDimensionPixelSize(R.styleable.CodeEditText_round, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 3, getResources().getDisplayMetrics()));
            Circle = t.getDimensionPixelSize(R.styleable.CodeEditText_circle, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 7, getResources().getDisplayMetrics()));
            textSize = t.getDimensionPixelSize(R.styleable.CodeEditText_textSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 16, getResources().getDisplayMetrics()));
            checkedColor = t.getColor(R.styleable.CodeEditText_checkedColor, 0xff44ce61);
            defaultColor = t.getColor(R.styleable.CodeEditText_defaultColor, 0xffd0d0d0);
            backColor = t.getColor(R.styleable.CodeEditText_backColor, 0xfff1f1f1);
            textColor = t.getColor(R.styleable.CodeEditText_textColor, 0xFF444444);
            waitInputColor = t.getColor(R.styleable.CodeEditText_waitInputColor, 0xFF444444);
            isPwd = t.getBoolean(R.styleable.CodeEditText_isPwd, true);
            isWaitInput = t.getBoolean(R.styleable.CodeEditText_isWaitInput, false);
            t.recycle();
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < textLength; i++) {
            ...
            RectF rect = new RectF(i * Wide + spzceX + paddingLeft, spzceY, i * Wide + Wide - spzceX + paddingLeft, Wide - spzceY); //四个值，分别代表4条线，距离起点位置的线
            ...
            if (isWaitInput && i == mText.length()) {  //显示待输入的线
                ...
                canvas.drawLine(i * Wide + Wide / 2 + paddingLeft, Wide / 2 - Wide / 5, i * Wide + Wide / 2 + paddingLeft, Wide / 2 + Wide / 5, l);
            }
        }
        //画密码圆点
